  ...
  useEffect(() => {
    ...
    if (socket.current) {
      ...
      socket.current.on("connectedUsers", ({ users }) => {
        ...
        console.log('Online users from socket 🏷🏷🏷')
      });
  ...
  useEffect(() => {
    const loadMessages = () => {
      ...
      socket.current.on("messagesLoaded", async ({ chat }) => {
        // messages with this  router.query.message
        setMessages(chat.messages);
        // chat with his image and his name from database
        setBannerData({
        ...
        // scroll to bottom of chat Mesages
        divRef.current && scrollDivToBottom(divRef);
      });

      socket.current.on("noChatFound", async () => {
        // if no chat yet with this user 
        // fetch his image and his name from database with getuserInfo
        const { name, profilePicUrl } = await getUserInfo(router.query.message);

        setBannerData({ name, profilePicUrl });
        // mesages is empty beacuse no conversation maked yet
        setMessages([]);
...
// console.log('error----<<<<' , errorLoading)
